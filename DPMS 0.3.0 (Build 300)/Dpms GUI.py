...
DEFAULT_GITHUB_API = "https://api.github.com/repos/discoveryos/Dpms--pkg/contents/packages"
DEFAULT_GITHUB_RAW = "https://raw.githubusercontent.com/discoveryos/Dpms--pkg/main/packages"
...
def get_local_packages():
    if not PACKAGE_DIR.exists():
        return []
    return [f.stem for f in PACKAGE_DIR.glob("*.tar.*")]

def get_github_packages():
    try:
        response = requests.get(DEFAULT_GITHUB_API)
        if response.status_code == 200:
            data = response.json()
            return [item['name'].rsplit('.', 2)[0] for item in data if item['name'].endswith(('.tar.xz', '.tar.gz'))]
        else:
            return []
    except Exception as e:
        print(f"Error fetching GitHub packages: {e}")
        return []
...
class DPMSGUI:
    ...
    def update_listbox(self, *args):
        ...
        combined_packages = list(set(get_local_packages() + get_github_packages()))
        for pkg in sorted(combined_packages):
    ...
    def install_selected(self):
        ...
        pkg_path = None
        for ext in ["tar.xz", "tar.gz"]:
            path = PACKAGE_DIR / f"{self.selected_package}.{ext}"
            if path.exists():
                pkg_path = path
                break

        if not pkg_path:
            pkg_path = self.download_package(self.selected_package)
            if not pkg_path:
                messagebox.showerror("Download Failed", "Package could not be downloaded.")
                return
